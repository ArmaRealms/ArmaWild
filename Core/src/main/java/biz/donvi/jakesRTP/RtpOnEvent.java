...
public class RtpOnEvent implements Listener {
    ...
    public RtpOnEvent(final RandomTeleporter randomTeleporter) {
        this.randomTeleporter = randomTeleporter;
    }

    private static void handlerLogging(final RegisteredListener[] registeredListeners, final String settingName, final String eventName) {
        int i = 0, us = 0;
    ...
    private static boolean locIntEqual(final Location a, final Location b) {
    ...
    public void playerJoin(final PlayerJoinEvent event) {
        ...
        try {
            ...
            final RandomTeleportAction rtpa = new RandomTeleportAction(
                    randomTeleporter,
    ...
    public void playerJoinMonitor(final PlayerJoinEvent event) {
        final Location properLoc = oldJoinEvents.get(event);
        final Location actualLoc = event.getPlayer().getLocation();
    ...
    @EventHandler(priority = EventPriority.MONITOR)
    public void playerRespawnMonitor(final PlayerRespawnEvent event) {
        final Location properLoc = oldRespawnEvents.get(event);
        final Location actualLoc = event.getRespawnLocation();
        if (properLoc == null || locIntEqual(properLoc, actualLoc)) return;
