...
import static biz.donvi.jakesRTP.evendistribuition.TransitionMatrices.ROTATIONS_0_90_180_270;
import static biz.donvi.jakesRTP.evendistribuition.TransitionMatrices.multiplyMatrixVector;
...
public class RandomCords {
     ...
     */
    static double randomG(final double shrink, final double center) {
     ...
     */
    public static int[] asIntArray2w(final double[] cordPair) {
        return new int[]{
                (int) cordPair[0],
                (int) cordPair[1]};
    }
     ...
     */
    public static double[] getRandXySquare(final int radiusMax, final int radiusMin) {
        final double[] randPairAtSize = new double[]{
    ...
    public static double[] getRandXyRectangle(final double rx, final double ry) {
        return new double[]{
                random() * rx * 2 - rx,
                random() * ry * 2 - ry};
    }

    public static double[] getRandXyRectangle(final double rx, final double ry, final double hrx, final double hry, final double hcx, final double hcy) {
        final double
                xb = hcx - hrx;
        final double xt = hcx + hrx;
        final double yb = hcy - hry;
        final double yt = hcy + hry;
        final double s0l = rx - xt;
        final double s0h = ry - yb;
        final double s1l = xt + rx;
        final double s1h = ry - yt;
        final double s2l = xb + rx;
        final double s2h = yt + ry;
        final double s3l = rx - xb;
        final double s3h = yb + ry;
        final double[] res = {random(), random()};
    ...
    private static int weightedChoice(final double... options) {
        double sum = 0;
        for (final double option : options) sum += option;
     ...
     * @return A pair of numbers, each randomly generated, that satisfy the above conditions.
     */
    public static double[] getRandXySquare(final int radiusMax, final int radiusMin, final double gShrink, final double gCenter) {
        final double s0 = (double) radiusMin / (double) radiusMax;
        final double s1 = 1 - s0;
        final double r0 = random();
     ...
     */
    public static double[] getRandXyCircle(final int radiusMax, final int radiusMin) {
     ...
     * @return A pair of numbers representing a random point inside the specific area in the circle.
     */
    public static double[] getRandXyCircle(final int radiusMax, final int radiusMin, final double gShrink, final double gCenter) {
        final double rMin2 = pow(radiusMin, 2d);
        final double r = sqrt((gShrink == 0 ? random() : randomG(gShrink, gCenter)) * (pow(radiusMax, 2d) - rMin2) + rMin2);
        final double theta = 2d * PI * random();
        final double x = (int) (r * cos(theta));
        final double y = (int) (r * sin(theta));
